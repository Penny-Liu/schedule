...
const PhysicianSchedulePage: React.FC<PhysicianSchedulePageProps> = ({
  currentUser,
}) => {
  // --- Copy Text Generators ---
  const generateBeitouCopyText = (
    date: Date,
    shifts: any[],
    doctors: Doctor[],
    staffShifts: any[],
    ...
    const dayHMShifts = db.getHealthMgmtShifts().filter((s) => {
      const shiftLoc =
        s.location || hmStaff.find((st) => st.id === s.userId)?.location;
      return s.date === dateStr && shiftLoc === "北投";
    });

    const mainRads = dayStaffShifts
      .filter((s) => {
    ...
    const mainHM = dayHMShifts
      .filter(
        (s) =>
          (s.task || "").includes("主控") || (s.station || "").includes("主控"),
      )
      .map((s) => {
        const st = hmStaff.find((t) => t.id === s.userId);
        return st?.alias || st?.name?.slice(-2);
      })
      .filter(Boolean);

    const assistRads = dayStaffShifts
      .filter((s) => {
        const u = users.find((user) => user.id === s.userId);
        return (
          u?.isRadiographer &&
    ...
    const assistHM = dayHMShifts
      .filter(
        (s) =>
          (s.task || "").includes("輔控") || (s.station || "").includes("輔控"),
      )
      .map((s) => {
        const st = hmStaff.find((t) => t.id === s.userId);
        return st?.alias || st?.name?.slice(-2);
    ...
    const hmPart =
      mainHM.length > 0 || assistHM.length > 0
        ? `  ${mainHM.join("/") || "-"}/${assistHM.join("/") || "-"}`
        : "";

    const getDocs = (station: string) =>
      dayShifts
        .filter((s) => s.scheduled_station === station || s.station === station)
        .map((s) => {
          const d = doctors.find((doc) => doc.id === s.doctorId);
          return d?.alias || d?.name || "?";
        });

    const giDocsArr = dayShifts
      .filter((s) => isGIStation(s.scheduled_station || s.station || ""))
      .map((s) => {
        const d = doctors.find((doc) => doc.id === s.doctorId);
        return d?.alias || d?.name || "?";
      });
...
GI：${stats?.beitou_gi || 0} 台`;
  ...
  const generateDazhiCopyText = (
    ...
    const getDocs = (station: string) =>
      dayShifts
        .filter((s) => s.scheduled_station === station)
        .map((s) => {
          const d = doctors.find((doc) => doc.id === s.doctorId);
          return d?.name || "?";
        });
    ...
    const giShifts = dayShifts.filter((s) =>
      isGIStation(s.scheduled_station || s.station || ""),
    );
    // Maintain a stable order for GI doctors based on the station name (GI1, GI2...)
    const sortedGIShifts = [...giShifts].sort((a, b) =>
      (a.scheduled_station || "").localeCompare(b.scheduled_station || ""),
    );
    const giDocsNames = sortedGIShifts.map(
      (s) => doctors.find((doc) => doc.id === s.doctorId)?.name || "?",
    );
    ...
    const mainHM = dayHMShifts
      ...
      .map((s) => hmStaff.find((t) => t.id === s.userId)?.name)
      .filter(Boolean);
    const assistHM = dayHMShifts
      .filter((s) => {
        const shiftLoc =
    ...
    const getHMByTask = (keyword: string): string[] => {
      return dayHMShifts
        .filter((s) => {
          ...
          return (
            shiftLoc === "大直" &&
            ((s.task || "").includes(keyword) ||
              (s.station || "").includes(keyword))
          );
        })
    ...
    const getHMNameForGI = (giStation: string): string => {
      const shift = dayHMShifts.find((s) => {
        ...
        return (
          shiftLoc === "大直" &&
          (s.task?.includes(giStation) || s.station?.includes(giStation))
        );
      });
    ...
    const anesDocNames = dayShifts
      .filter((s) => (s.scheduled_station || "").includes("麻"))
      .map((s) => doctors.find((d) => d.id === s.doctorId)?.name)
      .filter(Boolean) as string[];

    // 麻護：anesthesia nurses for 大直, alternately assigned across GI sessions
    const anesShifts = db
      .getAnesthesiaShifts()
      .filter((s) => s.date === dateStr && s.location === "大直");
    const anesNurseNames = anesShifts
      .map(
        (s) => db.getAnesthesiaStaff().find((as) => as.id === s.userId)?.name,
      )
      .filter(Boolean) as string[];
    ...
    const getSpec = (st: string) => {
      ...
      if (!res && st === "婦科") {
        const expShifts = dayShifts.filter(
          (s) => s.scheduled_station === "解說",
        );
        const doc = expShifts
          .map((s) => doctors.find((d) => d.id === s.doctorId))
          .find((d) => d?.capabilities?.includes("婦科"));
        if (doc) res = doc.name;
      }
      return res || "-";
    };
    ...
    // 動態項目
    const nutriNames = ["營1", "營2", "營3"]
      .flatMap((k) => getHMByTask(k))
      .join("、");
    ...
    const giScopeHeader = (() => {
      ...
      if (sortedGIShifts.length === 1) {
        const docName =
          doctors.find((d) => d.id === sortedGIShifts[0].doctorId)?.name || "?";
...
${flowWashNames ? "流+洗：" + flowWashNames : ""}${flowWashNames && (flowNames || washNames) ? "\n" : ""}${flowNames ? "流動：" + flowNames : ""}${flowNames && washNames ? "\n" : ""}${washNames ? "洗滌：" + washNames : ""}`;
  ...
  const canEdit =
    (currentUser.permissions?.includes(PERMISSIONS.EDIT_PHYSICIAN) ||
      currentUser.role === UserRole.SYSTEM_ADMIN) &&
    !isLocked;
  // Viewer and Finance can see stats only
  const isDazhiHMStaff =
    currentUser?.isHealthMgmt && currentUser?.healthMgmtLocation === "大直";
  const canEditStats =
    (currentUser.permissions?.includes(PERMISSIONS.VIEW_DOCTOR_STATS) ||
      currentUser.role === UserRole.SYSTEM_ADMIN ||
  ...
  const currentCycle = useMemo(() => {
    ...
    return (
      db.getCycles().find((c) => c.name === monthStr) ||
      db
        .getCycles()
        .find((c) => c.startDate.startsWith(monthStr.replace("/", "-")))
    );
  }, [currentDate]);
  ...
  const [stations, setStations] = useState<DoctorStationConfig[]>(() => {
    ...
    if (typeof currentList[0] === "string") {
      currentList = (currentList as any[]).map((s) => ({
        name: s,
        location: ["眼科", "耳鼻喉科", "婦科"].includes(s)
          ? "台中"
          : ["支援"].includes(s)
            ? "大直"
            : "北投",
  ...
  const [editData, setEditData] = useState<{
    ...
    location: string;
    task: string;
  }>({ station: "", workTime: "", note: "", location: "", task: "" });
  const [viewMode, setViewMode] = useState<
    "personnel" | "station" | "daily" | "statistics"
  >(() => {
  ...
  useEffect(() => {
    if (hiddenDoctorIds.length >= 0) {
      localStorage.setItem(
        `hiddenDoctorIds_${currentYearMonth}`,
        JSON.stringify(hiddenDoctorIds),
      );
    }
  }, [hiddenDoctorIds, currentYearMonth]);

  const updateQuickAssignData = (
    data: Partial<{
      station: string;
      location: string;
      workTime: string;
  ...
  const [simulatedShifts, setSimulatedShifts] = useState<DoctorShift[] | null>(
    null,
  );
  ...
  const activeShifts = useMemo(() => {
    ...
    if (isSimulationMode) {
      ...
      baseShifts = shifts.filter((s) => !s.isAutoGenerated);
    }
    // Filter out shifts of doctors that are no longer active (deleted)
    const activeDoctorIds = new Set(doctors.map((d) => d.id));
    return baseShifts.filter((s) => activeDoctorIds.has(s.doctorId));
  }, [shifts, isSimulationMode, doctors]);
  ...
  const handleAssignDoctor = async (doctorId: string) => {
    ...
    const contextShifts = isSimulationMode
      ? simulatedShifts || []
      : activeShifts;
    const doc = doctors.find((d) => d.id === doctorId);
    const existingShift = contextShifts.find(
      (s) => s.doctorId === doctorId && s.date === assignModal.date,
    );
    ...
    if (isSimulationMode) {
      ...
      const newSimShift: DoctorShift = {
        ...
        station:
          assignModal.station === "晚班"
            ? existingShift?.scheduled_station || "影像"
            : assignModal.station,
        scheduled_station:
          assignModal.station === "晚班"
            ? existingShift?.scheduled_station || "影像"
      ...
      const updated = [...(simulatedShifts || [])].filter(
        (s) => !(s.doctorId === doctorId && s.date === assignModal.date),
      );
    ...
    // 檢查月份是否已鎖定
    const assignYearMonth = assignModal.date.slice(0, 7); // YYYY-MM
    if (db.isMonthLocked(assignYearMonth)) {
      alert("該月份已鎖定，無法修改排班！");
      setAssignModal(null);
      return;
    }
    ...
    if (existingShift && confirm(confirmMsg)) {
      if (assignModal.station === "晚班") {
        await db.assignDoctorSchedule(
          doctorId,
          assignModal.date,
          existingShift.scheduled_station,
          existingShift.workTime,
          existingShift.note,
  ...
  const handleCellClick = async (doctorId: string, date: string) => {
    ...
    const contextShifts = isSimulationMode
      ? simulatedShifts || []
      : activeShifts;
    const shift = contextShifts.find(
    ...
    if (isQuickAssignMode) {
      ...
      await db.assignDoctorSchedule(
        doctorId,
        date,
        quickAssignData.station,
        quickAssignData.workTime,
        undefined,
        quickAssignData.location,
        quickAssignData.task,
      );
      setShifts(db.getDoctorShifts());
      return;
    }
  ...
  const handleSave = async () => {
    ...
    if (isSimulationMode) {
      const updated = [...(simulatedShifts || [])].filter(
        (s) =>
          !(
            s.doctorId === selectedCell.doctorId && s.date === selectedCell.date
          ),
      );
      if (editData.station) {
        updated.push({
    ...
    try {
      ...
      const tableConfig: any = {
        ...
        styles: {
          ...
          cellPadding:
            viewMode === "station"
              ? { top: 0.5, right: 0.3, bottom: 0.5, left: 0.3 }
              : 0.1,
          valign: "middle",
          halign: "center",
          lineWidth: 0.2,
        ...
        headStyles: {
          ...
          cellPadding:
            viewMode === "station"
              ? { top: 1.5, right: 0.5, bottom: 1.5, left: 0.5 }
              : 0.1,
            ...
            if (viewMode === "station") {
              ...
              if (data.column.index > 0) {
                ...
                if (raw && raw.rawShift) {
                  ...
                  if (rawText.includes("北") || raw.rawShift.location === "北")
                    data.cell.styles.fillColor = [220, 235, 255];
                  if (rawText.includes("直") || raw.rawShift.location === "直")
                    data.cell.styles.fillColor = [240, 220, 200];
                  if (rawText.includes("中") || raw.rawShift.location === "中")
                    data.cell.styles.fillColor = [255, 248, 186]; // Yellow for 台中
        ...
        didDrawCell: function (data: any) {
          ...
          if (viewMode !== "station" && data.section === "body") {
            ...
            if (
              radiologistStartIndex !== -1 &&
              rowIndex === radiologistStartIndex
            ) {
            ...
            if (
              radiologistEndIndex !== -1 &&
              rowIndex === radiologistEndIndex
            ) {
              doc.setLineWidth(0.5);
              doc.setDrawColor(0, 0, 0);
              doc.line(
      ...
      if (viewMode === "station") {
        ...
        LOCATIONS.forEach((loc, locIndex) => {
          ...
          if (loc === "北投") {
            const isHM = (userId: string) =>
              healthMgmtStaff.some((s) => s.id === userId);
            const isRad = (userId: string) =>
            ...
            dateRange.forEach((date) => {
              ...
              const radMainShifts = dayRadShifts.filter(
                (s) =>
                  s.station?.includes("場控") ||
                  s.station === "主" ||
                  s.station === "主控",
              );
              const radAssistShifts = dayRadShifts.filter(
                (s) => s.specialRoles?.includes("輔班") || s.station === "輔控",
              );
          ...
          locStations.forEach((s) => {
            ...
            if (!processedStationNames.includes(name))
              processedStationNames.push(name);
          });
            ...
            dateRange.forEach((date) => {
              ...
              if (stName === "GI") {
                ["GI1", "GI2", "GI", "腸胃"].forEach((actualSt) => {
                  const subShifts = shifts.filter(
                    (s) =>
                      s.date === date &&
                      s.location === loc &&
                      s.scheduled_station === actualSt,
                  );
              ...
              if (
                ...
                ["遠班", "遠距", "遠"].includes(stName)
              ) {
                if (loc === "大直") {
                  assignedShifts = shifts.filter(
                    (s) =>
                      s.date === date &&
                      s.location === "北投" &&
                      ["遠班", "遠距", "遠"].includes(
              ...
              if (assignedShifts.length > 0) {
                ...
                const hasAnyTask = assignedShifts.some(
                  (s) => s.task && !(stName === "晚班" && s.task === "晚班"),
                );
                const paddingBuffer = hasAnyTask ? 2.5 : 1.0;
                cellStyles = {
                  minCellHeight: Math.max(
                    totalHeight + paddingBuffer,
                    dynamicMinHeight,
                  ),
                };
              } else {
          ...
          const rowData: any[] = [
            {
              ...
              styles: {
                fontStyle: "bold",
                fontSize: 9,
                minCellHeight: fixedHeight,
                cellPadding: { top: 0.2, bottom: 0.2, left: 0, right: 0 },
              },
            },
          ];
          dateRange.forEach((date) => {
            ...
            const isExcluded = doc.excludedDays?.includes(
            ...
            if (shift) {
              ...
              if (st === "X") {
                ...
                const allShiftsForDate = shifts.filter(
                  (s) => s.doctorId === doc.id && s.date === date,
                );
                const hasGynecology = allShiftsForDate.some(
                  (s) => s.scheduled_station === "婦科",
                );
                const hasExplanation = allShiftsForDate.some(
                  (s) => s.scheduled_station === "解說",
                );
                let displayStation =
                  hasGynecology && hasExplanation ? "解+婦" : st;
  ...
  const handleExportExcel = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      const addLocationSection = (locationName: string, colorHex: string) => {
        ...
        const locStations = db.settings.doctorStations.filter(
          (ds) => ds.location === locationName,
        );
        ...
        locRow.getCell(1).font = {
          ...fontBase,
          size: 14,
          bold: true,
          color: { argb: "FFFFFFFF" },
        };
        locRow.getCell(1).alignment = {
          vertical: "middle",
          horizontal: "center",
        };
        ...
        if (locationName === "北投") {
          ...
          dateRange.forEach((dateStr, colIdx) => {
            const dayRadShifts = radShifts.filter(
              (s) =>
                s.date === dateStr &&
                (s.station?.includes("場控") ||
                  s.station === "主" ||
            ...
            const dayHMShifts = allHMShifts.filter((s) => {
              ...
              if (!(s.task === "主控" || s.station?.includes("主控")))
                return false;
              const st = healthMgmtStaff.find((u) => u.id === s.userId);
              const effectiveLoc = s.location || st?.location || "北投";
              return effectiveLoc === "北投";
            ...
            const names = [
              ...dayRadShifts.map(
                (s) => radUsers.find((u) => u.id === s.userId)?.name,
              ),
              ...dayHMShifts.map(
                (s) => healthMgmtStaff.find((u) => u.id === s.userId)?.name,
              ),
            ].filter(Boolean);
          ...
          dateRange.forEach((dateStr, colIdx) => {
            ...
            const dayHMShifts = allHMShifts.filter((s) => {
              if (s.date !== dateStr) return false;
              if (!(s.task === "輔控" || s.station?.includes("輔控")))
                return false;
              const st = healthMgmtStaff.find((u) => u.id === s.userId);
              const effectiveLoc = s.location || st?.location || "北投";
              return effectiveLoc === "北投";
          ...
          dateRange.forEach((dateStr, colIdx) => {
            ...
            const names = dayHMShifts
              .map((s) => healthMgmtStaff.find((u) => u.id === s.userId)?.name)
              .filter(Boolean);
          ...
          dateRange.forEach((dateStr, colIdx) => {
            ...
            if (stationShifts.length > 0) {
              ...
              stationShifts.forEach((shift, idx) => {
                ...
                const time = formatTimeForExcel(
                  shift.workTime || (shift as any).work_time || "",
                );

                // User Request: Station View also needs Work Time and Task
                const showTaskValue =
                  shift.task && !shift.task.includes("固定");
                const task = showTaskValue ? `(${shift.task})` : "";

                const baseColor =
                  isSimulationMode && shift.isAutoGenerated
                    ? "FFD97706"
                    : undefined;
                ...
                if (task) {
                  const taskColor =
                    shift.task === "晚班" ? "FFFF0000" : "FF0000FF"; // Red for Evening, Blue for others
                  richText.push({
                    text: "\n" + task,
      ...
      doctors.forEach((doc) => {
        ...
        dateRange.forEach((dateStr, colIdx) => {
          ...
          // Find shift
          const shift =
            (simulatedShifts || []).find(
              (s) => s.doctorId === doc.id && s.date === dateStr,
            ) ||
          ...
          if (
            ...
            if (shift.scheduled_station === "解說") {
              ...
              if (dayShifts.some((s) => s.scheduled_station === "婦科"))
                stationVal = "解+婦";
            }
  ...
  const getShiftDisplay = (doctorId: string, date: string) =>
    shifts.find((s) => s.doctorId === doctorId && s.date === date);
  ...
  const handleAutoSchedule = async () => {
    ...
    try {
      ...
      const result = await db.autoScheduleDoctors(
        startDate,
        endDate,
        targetDays,
        false,
  ...
  const handleSaveSimulation = async () => {
    ...
    try {
      ...
      // 記錄操作日誌
      db.logOperation("save_simulation", "physician", {
        dateRange: {
          start: simulatedShifts[0]?.date || "",
          end: simulatedShifts[simulatedShifts.length - 1]?.date || "",
        },
        affectedCount: simulatedShifts.length,
        note: "儲存模擬排班結果",
      });
    } catch (e) {
      console.error(e);
  ...
  const handleAutoScheduleDay = async () => {
    if (
      !confirm(
        `確定要為 ${toLocalISOString(currentDate)} 執行自動排班嗎？\n(將會自動填補空缺)`,
      )
    )
      return;
    setIsAutoScheduling(true);
  ...
  const handleClearDay = async () => {
    ...
    const manualShifts = dayShifts.filter((s) => !s.isAutoGenerated);

    let confirmMsg = `⚠️ 警告：確定要清空 ${targetDate} 的所有排班嗎？\n此動作無法復原！`;
    if (manualShifts.length > 0) {
      confirmMsg += `\n\n注意：此日期有 ${manualShifts.length} 個手動排班記錄將被刪除！`;
    }

    if (!confirm(confirmMsg)) return;

    try {
      await db.clearDoctorShifts(targetDate, targetDate);
      alert("已清空單日排班");
    } catch (error) {
      console.error(error);
      alert("清除失敗");
        ...
        <div className="bg-amber-100 border-b border-amber-200 px-4 py-2 flex items-center justify-between shadow-sm sticky top-0 z-50 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex items-center gap-2 text-amber-800">
            ...
            <div>
              ...
              <span className="text-sm ml-2 hidden sm:inline">
                這是自動生成的建議排班，點擊「儲存」才會正式生效。
              </span>
            </div>
          </div>
            ...
            <>
              ...
              {isQuickAssignMode && (
                <div className="ml-1 flex items-center gap-1 px-2 py-1 bg-amber-50 border border-amber-300 rounded-lg text-xs">
                  <select
                    ...
                    onChange={(e) =>
                      updateQuickAssignData({ station: e.target.value })
                    }
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateQuickAssignData({ location: e.target.value })
                    }
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateQuickAssignData({ workTime: e.target.value })
                    }
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateQuickAssignData({ task: e.target.value })
                    }
                    className="w-16 bg-transparent text-amber-700 outline-none placeholder:text-amber-300/50"
                    title="手動輸入特殊任務"
                  />
              ...
              >
                <ArrowUpDown
                  size={14}
                  className={
                    showSpecialtyOrderModal ? "text-white" : "text-gray-400"
                  }
                />
                科別排序
              </button>
              ...
              <button
                onClick={() => {
                  if (
                    confirm(
                      "確定要依照設定的科別順序，重新排列所有醫師的順序嗎？\n(這將會更新資料庫中的排序設定)",
                    )
                  ) {
                    db.resortDoctorsBySpecialty().then(() =>
                      alert("已完成排序執行！"),
                    );
                  }
                }}
              ...
              >
                <ArrowUpDown
                  size={14}
                  className={isReorderMode ? "text-white" : "text-gray-400"}
                />
                {isReorderMode ? "完成" : "調序"}
              </button>
            ...
            <table className="text-sm border-collapse w-auto">
              <thead className="relative z-50">
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="p-2 md:p-2 text-left font-bold text-slate-600 w-[70px] md:w-[90px] sticky left-0 top-0 bg-slate-50 z-[60] border-r border-slate-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                    醫師
                  </th>
                  {dateRange.map((date) => {
                    const d = new Date(date);
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`font-bold text-[11px] leading-tight ${isToday ? "text-teal-600" : isHoliday || isWeekend ? "text-red-500" : "text-slate-800"}`}
                        >
                          {d.getMonth() + 1}/{d.getDate()}
                        </div>
                        <div
                          className={`text-[10px] opacity-75 leading-tight ${isToday ? "text-teal-600" : isHoliday || isWeekend ? "text-red-500" : "text-slate-700"}`}
                        >
                          {
                            ["日", "一", "二", "三", "四", "五", "六"][
                              d.getDay()
                        ...
                        {(holiday || note || doctorNote) && (
                          <div className="flex flex-col items-center">
                            {holiday && (
                              <div
                                className="text-[8px] font-bold text-red-600 whitespace-nowrap overflow-hidden text-ellipsis max-w-[40px]"
                                title={holiday.name}
                              >
                                {holiday.name}
                              </div>
                            ...
                            {note && (
                              <div
                                className="text-[8px] font-medium text-blue-600 whitespace-nowrap overflow-hidden text-ellipsis max-w-[40px]"
                                title={note.name}
                              >
                                {note.name}
                              </div>
                            ...
                            {doctorNote && (
                              <div
                                className="text-[9px] bg-purple-100 text-purple-700 px-0.5 rounded-lg font-bold shadow-sm whitespace-nowrap overflow-hidden text-ellipsis max-w-[40px]"
                                title={doctorNote.name}
                              >
                                📝 {doctorNote.name}
                              </div>
                            )}
                ...
                {filteredDoctorsForDisplay.map((doc) => (
                  ...
                  >
                      ...
                      <div className="p-2 md:p-2 font-bold text-slate-800 flex items-center gap-1 md:gap-2 min-w-[70px] md:min-w-[90px]">
                        {isReorderMode && (
                          <div className="flex flex-col gap-0.5">
                            ...
                            <button
                              ...
                              className="text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-lg p-0.5 transition-colors disabled:opacity-20 disabled:cursor-not-allowed"
                              disabled={
                                doctors.indexOf(doc) === doctors.length - 1
                              }
                              title="向下移動"
                            >
                              <svg
                        ...
                        <div className="flex flex-col">
                          ...
                          {(() => {
                            ...
                            const allShifts = isSimulationMode
                              ? [...activeShifts, ...(simulatedShifts || [])]
                              : shifts;
                            ...
                            if (
                              doc.personalCycles &&
                              doc.personalCycles[currentMonthStr]
                            ) {
                            ...
                            if (activeCycle) {
                              const cycleShifts = allShifts.filter(
                                (s) =>
                                  s.doctorId === doc.id &&
                              ...
                              return (
                                <span className="text-[10px] text-slate-400 font-normal leading-tight">
                                  {fmtDate(activeCycle.startDate)}~
                                  {fmtDate(activeCycle.endDate)} · {cycleShifts}
                                  天
                                </span>
                              );
                            }
                            // Fallback: current month count in dateRange
                            const monthCount = allShifts.filter(
                              (s) =>
                                s.doctorId === doc.id &&
                                dateRange.includes(s.date) &&
                                !!s.scheduled_station &&
                    ...
                    {dateRange.map((date) => {
                      ...
                      const simulatedShift = simulatedShifts?.find(
                        (s) => s.doctorId === doc.id && s.date === date,
                      );
                      const shift =
                        simulatedShift ||
                      ...
                      if (isGIStation(cellDisplayStation))
                        cellDisplayStation = "GI";
                      else if (cellDisplayStation === "耳鼻喉科")
                        cellDisplayStation = "ENT";
                      ...
                      if (shift?.scheduled_station === "解說") {
                        ...
                        if (
                          allShiftsForDate.some(
                            (s) => s.scheduled_station === "婦科",
                          )
                        )
                          cellDisplayStation = "解+婦";
                      }
                      ...
                      return (
                        <td
                          key={date}
                          onClick={() =>
                            canEdit && handleCellClick(doc.id, date)
                          }
                                                          ...
                                                          hasStation
                                                            ? (() => {
                                                                ...
                                                                if (
                                                                  ...
                                                                  return "bg-[#D7CCC8] hover:bg-[#BCAAA4]";
                          ...
                          {shift ? (
                            shift.scheduled_station === "X" ? (
                              <div className="h-full w-full flex items-center justify-center bg-slate-100/70 pattern-diagonal-lines pattern-slate-300 pattern-bg-transparent pattern-opacity-40">
                                <span className="text-slate-400 text-lg font-bold select-none">
                                  ×
                                </span>
                              </div>
                            ) : (
                                ...
                                >
                                  {(() => {
                                    ...
                                    const displayStation =
                                      hasGynecology && hasExplanation
                                        ? "解+婦"
                                        : shift.scheduled_station === "耳鼻喉科"
                                          ? "ENT"
                                          : shift.scheduled_station || "";

                                    return (
                                      <span className="font-bold text-teal-700 block text-xs md:text-sm leading-tight text-center">
                                        {displayStation}
                                      </span>
                                    );
                                  })()}
                                  ...
                                  {shift.location && (
                                    <div
                                      className={`text-[10px] px-1 rounded-lg text-white scale-90 ${LOCATION_COLORS[shift.location]?.split(" ")[0] || "bg-gray-400"}`}
                                    >
                                      {shift.location}
                                    </div>
                                  )}
                                  {shift.note && (
                                    <div
                                      className="text-[8px] text-slate-500 scale-90 w-full text-center overflow-hidden text-ellipsis whitespace-nowrap px-0.5"
                                      title={shift.note}
                                    >
                                      {shift.note.length > 4
                                        ? shift.note.substring(0, 4) + ".."
                                        : shift.note}
                                    </div>
                                  )}
                                  ...
                                  {(() => {
                                    ...
                                    const weekdaySetting =
                                      doc.weekdaySettings?.find(
                                        (ws) => ws.dayOfWeek === dayOfWeek,
                                      );
                                    // Only show weekday task if it's DIFFERENT from the shift task to avoid duplication
                                    if (
                                      weekdaySetting?.task &&
                                      weekdaySetting.task !== shift.task
                                    ) {
                                      return (
                                        <div
                                          className="text-[8px] text-blue-400 font-medium scale-90 w-full text-center overflow-hidden text-ellipsis whitespace-nowrap px-0.5 opacity-75"
                                          title={`${["日", "一", "二", "三", "四", "五", "六"][dayOfWeek]}任務: ${weekdaySetting.task}`}
                                        >
                                          ({weekdaySetting.task})
                                        </div>
                                      );
                            ...
                            <div className="h-full w-full flex items-center justify-center">
                              <span className="text-slate-300 text-lg font-light select-none">
                                ×
                              </span>
                            </div>
                          ) : (
                            <div
                              className={`opacity-0 group-hover:opacity-100 flex justify-center transition-opacity ${!canEdit ? "hidden" : ""}`}
                            >
                              {isQuickExcludeMode ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm inline-block min-w-full">
            <table className="w-auto border-collapse bg-white">
              <thead className="sticky top-0 z-50 shadow-sm">
                <tr>
                  <th
                    className={`sticky top-0 left-0 z-[90] align-top bg-slate-50 border-b border-r border-slate-200 shadow-[4px_0_8px_rgba(0,0,0,0.02)] ${isMobile ? "p-1 w-[70px] min-w-[70px]" : "p-2 w-[96px] text-left"}`}
                  >
                    <div
                      className={`flex items-start font-bold text-xs text-slate-700 ${isMobile ? "justify-center" : "gap-2"}`}
                    >
                  ...
                  {dateRange.map((date) => {
                    ...
                    const dailyHolidays = db
                      .getHolidays()
                      .filter((h) => h.date === date);
                    const holiday = dailyHolidays.find(
                      (h) =>
                        h.type === DateEventType.NATIONAL ||
                        h.type === DateEventType.CLOSED,
                    );
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col items-center gap-0.5">
                          ...
                          <div
                            className={`text-[9px] opacity-75 leading-tight ${isToday ? "text-teal-600" : isWeekend || holiday ? "text-red-500" : "text-slate-700"}`}
                          >
                            {
                              ["日", "一", "二", "三", "四", "五", "六"][
                                d.getDay()
                          ...
                          {holiday && (
                            <span
                              className={`text-[8px] px-1 rounded-sm leading-tight max-w-[38px] truncate mt-0.5 ${isToday ? "bg-teal-200 text-teal-800 border border-teal-300" : "bg-red-100 text-red-700 border border-red-200"}`}
                            >
                              {holiday.name}
                            </span>
                          )}
              ...
              <tbody className="divide-y divide-slate-100">
                {/* --- Manpower Stats Rows --- */}
                <tr
                  className="bg-slate-100/50 border-t-2 border-slate-200 cursor-pointer hover:bg-slate-100 transition-colors"
                  onClick={() =>
                    setIsManpowerStatsExpanded(!isManpowerStatsExpanded)
                  }
                >
                  <td colSpan={dateRange.length + 1} className="py-1.5 px-3">
                    <div className="flex items-center gap-2 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                      ...
                      客戶與檢查統計 (北投/大直)
                      {!isManpowerStatsExpanded && (
                        <span className="text-[10px] font-normal lowercase text-slate-400 ml-2">
                          (點擊展開詳細數據)
                        </span>
                      )}
                    </div>
                ...
                {isManpowerStatsExpanded && (
                  <>
                    <tr className="bg-slate-50">
                      <td
                        className={`sticky left-0 z-10 bg-slate-50 border-r border-slate-200 shadow-[2px_0_4px_rgba(0,0,0,0.02)] ${isMobile ? "p-1 w-[85px] min-w-[85px]" : "p-2"}`}
                      >
                        <div className="text-[10px] font-bold text-slate-600 flex items-center justify-end pr-2">
                          北投客戶數
                        </div>
                      </td>
                      {dateRange.map((date) => {
                        ...
                        return (
                          <td
                            key={date}
                            className="p-0.5 border-r border-slate-100 text-center align-middle"
                          >
                            {canEditStats ? (
                              <input
                                type="number"
                                ...
                                onChange={(e) =>
                                  db.updateDailyStats(date, {
                                    beitou_clients: Number(e.target.value),
                                  })
                                }
                          ...
                          >
                            {canEditStats ? (
                              <input
                                ...
                                onChange={(e) =>
                                  db.updateDailyStats(date, {
                                    beitou_mr: Number(e.target.value),
                                  })
                                }
                                className="w-full text-center text-xs font-bold bg-transparent outline-none focus:bg-white focus:ring-1 focus:ring-teal-500 rounded-lg py-1 text-slate-700"
                                placeholder="0"
                          ...
                          >
                            {canEditStats ? (
                              <input
                                ...
                                onChange={(e) =>
                                  db.updateDailyStats(date, {
                                    beitou_gi: Number(e.target.value),
                                  })
                                }
                                className="w-full text-center text-xs font-bold bg-transparent outline-none focus:bg-white focus:ring-1 focus:ring-teal-500 rounded-lg py-1 text-slate-700"
                                placeholder="0"
                          ...
                          >
                            {canEditStats ? (
                              <input
                                ...
                                onChange={(e) =>
                                  db.updateDailyStats(date, {
                                    beitou_cta: Number(e.target.value),
                                  })
                                }
                                className="w-full text-center text-xs font-bold bg-transparent outline-none focus:bg-white focus:ring-1 focus:ring-teal-500 rounded-lg py-1 text-slate-700"
                                placeholder="0"
                    ...
                    <tr className="bg-slate-50">
                      <td
                        className={`sticky left-0 z-10 bg-slate-50 border-r border-slate-200 shadow-[2px_0_4px_rgba(0,0,0,0.02)] ${isMobile ? "p-1 w-[85px] min-w-[85px]" : "p-2"}`}
                      >
                        <div className="text-[10px] font-bold text-slate-600 flex items-center justify-end pr-2 text-right">
                          大直健檢
                          <br />
                          客戶數
                        </div>
                          ...
                          >
                            {canEditStats ? (
                              <input
                                ...
                                onChange={(e) =>
                                  db.updateDailyStats(date, {
                                    dazhi_clients: Number(e.target.value),
                                  })
                                }
                                className="w-full text-center text-xs font-bold bg-transparent outline-none focus:bg-white focus:ring-1 focus:ring-teal-500 rounded-lg py-1 text-slate-700"
                                placeholder="0"
                          ...
                          >
                            {canEditStats ? (
                              <input
                                ...
                                onChange={(e) =>
                                  db.updateDailyStats(date, {
                                    dazhi_gi: Number(e.target.value),
                                  })
                                }
                                className="w-full text-center text-xs font-bold bg-transparent outline-none focus:bg-white focus:ring-1 focus:ring-teal-500 rounded-lg py-1 text-slate-700"
                                placeholder="0"
                ...
                {LOCATIONS.map((location) => {
                  ...
                  const locationStations = stations.filter(
                  ...
                  return (
                    <React.Fragment key={location}>
                      ...
                      >
                        <td
                          colSpan={dateRange.length + 1}
                          className="px-3 py-2 font-bold text-slate-800 bg-slate-200 sticky left-0 z-10 text-left border-y-2 border-slate-400 shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)]"
                        >
                          <div className="flex items-center justify-between w-full">
                            <div className="flex items-center gap-2">
                              <span
                                className={`w-2.5 h-2.5 rounded-full ${LOCATION_COLORS[location]?.split(" ")[0]}`}
                              ></span>
                        ...
                        <>
                          ...
                          {(location === "北投" || location === "大直") && (
                            <>
                              ...
                              <tr className="bg-amber-50/30 border-b border-amber-100 group hover:bg-amber-50/50 transition-colors">
                                <td
                                  className={`sticky left-0 z-10 bg-amber-50/80 backdrop-blur border-r border-amber-200 shadow-[4px_0_8px_rgba(0,0,0,0.02)] ${isMobile ? "p-1 w-[85px] min-w-[85px]" : "p-2"}`}
                                >
                                  <div className="text-xs font-bold text-amber-800 flex items-center justify-end pr-2">
                                    主班
                                  </div>
                                </td>
                                {dateRange.map((date) => {
                          ...
                          {(() => {
                            ...
                            const orderedStationNames =
                              PRESCRIBED_ORDER[location] || [];
                            const locationStations = orderedStationNames.map(
                              (name) => ({ name, location }),
                            );
                            ...
                            return locationStations.map((stationConfig) => {
                              ...
                              return (
                                ...
                                >
                                  {/* Station Name Header */}
                                  <td
                                    className={`sticky left-0 z-10 bg-white group-hover:bg-slate-50 backdrop-blur-sm border-r border-slate-200 shadow-[4px_0_8px_rgba(0,0,0,0.02)] ${isMobile ? "p-1 w-[85px] min-w-[85px]" : "p-2"}`}
                                  >
                                  ...
                                  {dateRange.map((date) => {
                                    ...
                                    const currentShifts =
                                      allRelevantShifts.filter((s) => {
                                        ...
                                        if (s.scheduled_station === stationName)
                                          return true;
                                        ...
                                        if (
                                          ...
                                          return doc?.capabilities?.includes(
                                            "婦科",
                                          );
                                        }
                                    ...
                                    if (
                                      ...
                                      const beitouRemoteShifts =
                                        allRelevantShifts.filter(
                                          (s) =>
                                            s.date === date &&
                                            s.location === "北投" &&
                                            ["遠班", "遠距", "遠"].includes(
                                    ...
                                    const getStationBgColor = () => {
                                      ...
                                      if (
                                        stationName.includes("腸胃") ||
                                        stationName.toLowerCase().includes("gi")
                                      )
                                        return "bg-blue-100/50";
                                      if (stationName.includes("解說"))
                                        return "bg-orange-100/50";
                                      if (stationName.includes("支援"))
                                        return "bg-yellow-100/50";
                                      if (stationName.includes("行政"))
                                        return "bg-white";
                                      if (
                                        stationName.includes("眼") ||
                                        stationName.includes("婦") ||
                                        stationName.includes("耳")
                                      )
                                    ...
                                    return (
                                      ...
                                      >
                                            ...
                                            return (
                                              <div className="absolute top-0 right-0 z-[1] bg-blue-700 text-white px-1.5 py-0.5 text-[10px] font-black rounded-bl shadow-md pointer-events-none">
                                                {location === "北投"
                                                  ...
                                                  : location === "大直"
                                                    ? `${stats?.dazhi_clients || 0}/${stats?.dazhi_metabolism_clients || 0}`
                                                    : ""}
                                              </div>
                                            );
                                          }
                                        ...
                                        {displayShifts.length > 0 ? (
                                          <div
                                            className={`flex flex-col items-center gap-0.5 px-0.5 pb-1 w-full ${isGIStation(stationName) || stationName.includes("解說") ? "justify-start pt-6" : "justify-center py-1"}`}
                                          >
                                            {displayShifts.map(
                                              (shift, index) => {
                                                const doc = doctors.find(
                                                ...
                                                const isSimulated =
                                                  simulatedShifts?.some(
                                                    (ss) => ss.id === shift.id,
                                                  );
                                                return (
                                                  <div
                                                    key={shift.id}
                                                    className={`w-full text-center p-0.5 rounded ${isSimulated ? "bg-amber-50 border border-dashed border-amber-400 animate-pulse" : ""}`}
                                                  >
                                                    <div
                                                      className={`flex flex-col items-center ${isSimulated ? "text-amber-800" : "text-slate-800"}`}
                                                      title={doc?.name}
                                                    >
                                                      <div
                                                        className={`text-[12px] md:text-[13px] font-normal leading-tight whitespace-nowrap tracking-tight ${isSimulated ? "text-amber-900" : "text-slate-900"}`}
                                                      >
                                                        {doc?.name || "?"}
                                                      </div>
                                                      {suffix && (
          ...
          <div className="flex flex-col gap-8 pb-10">
            {(() => {
              ...
              return LOCATIONS.map((loc) => {
                const rawLocStations = stations.filter(
                  (s) => s.location === loc,
                );
                      ...
                      <div className="p-5 md:p-6 flex flex-col gap-6 bg-slate-50/30">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                          {locStations.map((config) => {
                            ...
                            const shiftsOnStation = allRelevantShifts.filter(
                              (s) => {
                                if (
                                  s.date !== toLocalISOString(currentDate) ||
                            ...
                            if (
                              ...
                              const beitouRemoteShifts = shifts.filter(
                                (s) =>
                                  s.date === toLocalISOString(currentDate) &&
                                  s.location === "北投" &&
                                  ["遠班", "遠距", "遠"].includes(
                            ...
                            if (
                              ...
                              const dazhiRemoteShifts = shifts.filter(
                                (s) =>
                                  s.date === toLocalISOString(currentDate) &&
                                  s.location === "大直" &&
                                  ["遠班", "遠距", "遠"].includes(
                            ...
                            if (st === "GI") {
                              displayShifts.sort((a, b) => {
                                ...
                                return (
                                  getWeight(
                                    a.scheduled_station || a.station || "",
                                  ) -
                                  getWeight(
                                    b.scheduled_station || b.station || "",
                                  )
                                );
                              });
                            }
                              ...
                              >
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    {isShort && (
                                      <span
                                        className="flex h-2 w-2 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)] animate-pulse"
                                        title="人力不足"
                                      ></span>
                                    )}
                                  </h3>
                                ...
                                <div className="flex-1 flex flex-col p-2 bg-slate-50/40 gap-1.5">
                                  {displayShifts.length > 0 ? (
                                    displayShifts.map((s) => {
                                      ...
                                      const isSimulated = simulatedShifts?.some(
                                        (ss) => ss.id === s.id,
                                      );
                                      return (
                                        <div
          ...
          <div className="p-4 max-w-full">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <h3 className="text-sm font-bold bg-green-50 px-4 py-2 border-b border-green-100 flex items-center gap-2 text-green-700">
                <FileText size={16} className="text-green-600" /> 複製 Line 文字
                (Copy to Line)
              </h3>
          ...
          <div className="flex flex-col gap-6 overflow-auto p-4 max-w-full">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <h3 className="text-sm font-bold bg-slate-50 px-4 py-2 border-b border-gray-100 flex items-center gap-2 text-slate-700">
                <BarChart2 size={16} className="text-teal-600" /> 每日崗位覆蓋率
                (實際人數 / 需求人數)
              </h3>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead className="bg-slate-50 text-slate-500 font-bold border-b border-slate-200">
                    <tr>
                      <th className="px-3 py-2 text-left sticky left-0 bg-slate-50 z-10 border-r border-slate-200 shadow-[2px_0_5px_rgba(0,0,0,0.05)]">
                        日期
                      </th>
                      {(() => {
                        ...
                        return stations
                          ...
                          .map((st) => {
                            const displayStationName = isGIStation(st.name)
                              ? "GI"
                              : st.name;
                            return (
                              <th
                                key={`head-${displayStationName}-${st.location}`}
                                className="px-2 py-2 text-center min-w-[60px] border-r border-gray-100"
                              >
                                <div className="flex flex-col items-center">
                                  <span>{displayStationName}</span>
                                  <span
                                    className={`text-[9px] px-1.5 rounded-full mt-0.5 text-white ${LOCATION_COLORS[st.location]?.split(" ")[0] || "bg-gray-400"}`}
                                  >
                                    {st.location}
                                  </span>
                                </div>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {dateRange.map((date) => {
                      ...
                      return (
                        <tr
                          key={date}
                          className={`hover:bg-slate-50 transition-colors ${isWeekend ? "bg-red-50/10" : ""}`}
                        >
                              ...
                              .map((st) => {
                                ...
                                if (displayStationName === "GI") {
                                  stations
                                    .filter(
                                      (os) =>
                                        os.location === st.location &&
                                        isGIStation(os.name),
                                    )
                                    .forEach((os) => {
                                      const reqKey = `${os.name}_${os.location}`;
                                      const reqs = requirements[reqKey] ||
                                        requirements[os.name] || [
                                          0, 0, 0, 0, 0, 0, 0,
                                        ];
                                      req += reqs[dayOfWeek] || 0;
                                    });
                                ...
                                return (
                                  <td
                                    key={`${date}-${displayStationName}-${st.location}`}
                                    className={`px-2 py-2 text-center border-r border-gray-50 font-mono font-bold ${isLow ? "bg-red-50" : ""}`}
                                  >
                                    <span
                                      className={
                                        isLow ? "text-red-600" : "text-teal-600"
                                      }
                                    >
                                      {count}
                                    </span>
                                    <span className="text-gray-300 mx-0.5 text-[10px]">
                                      /
                                    </span>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <h3 className="text-base font-bold bg-slate-50 px-4 py-3 border-b border-gray-100 flex items-center gap-2 text-slate-700">
                <Briefcase size={18} className="text-indigo-600" /> 醫師排班統計
                (班數)
              </h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 text-slate-500 font-bold border-b border-slate-200">
                    <tr>
                      <th className="px-3 py-3 text-left sticky left-0 bg-slate-50 z-10 border-r border-slate-200 shadow-[2px_0_5px_rgba(0,0,0,0.05)] w-32">
                        醫師姓名
                      </th>
                      <th className="px-2 py-3 text-center bg-teal-50 text-teal-700 border-r border-teal-100 font-extrabold w-16">
                        週期
                      </th>
                      <th className="px-2 py-3 text-center bg-indigo-50 text-indigo-700 border-r border-indigo-100 font-extrabold w-16">
                        總計
                      </th>
                      {(() => {
                        const seen = new Set<string>();
                        return stations
                          ...
                          .map((st) => {
                            ...
                            return (
                              <th
                                key={`doc-head-${displayStationName}-${st.location}`}
                                className="px-2 py-3 text-center min-w-[60px] border-r border-gray-100 font-normal"
                              >
                                <div className="flex flex-col items-center opacity-80">
                                  ...
                                  <span
                                    className={`text-[10px] px-1.5 py-0.5 rounded-full mt-0.5 text-white ${LOCATION_COLORS[st.location]?.split(" ")[0] || "bg-gray-400"}`}
                                  >
                                    {st.location}
                                  </span>
                                </div>
                    ...
                    {doctors.map((doc) => {
                      ...
                      const isActualWork = (station?: string) => {
                        ...
                        if (
                          sNormalized === "" ||
                          sNormalized === "未分配" ||
                          sNormalized === "UNASSIGNED"
                        )
                          return false;
                        if (
                          sNormalized === "休假" ||
                          sNormalized.startsWith("休")
                        )
                          return false;
                        if (sNormalized === "X" || sNormalized === "SYSTEMOFF")
                          return false;
                        return true;
                      };
                      ...
                      const docShifts = shifts.filter(
                        (s) =>
                          s.doctorId === doc.id &&
                          currentCycleRange.includes(s.date),
                      );
                      ...
                      stations
                        ...
                        .forEach((st) => {
                          ...
                          const count = docShifts.filter((s) => {
                            ...
                            if (displayStationName === "GI") {
                              return (
                                isGIStation(sStation) &&
                                s.location === st.location
                              );
                            }
                            return (
                              sStation === displayStationName &&
                              s.location === st.location
                            );
                          }).length;
                          total += count;
                        ...
                        >
                          <td className="px-3 py-3 text-left sticky left-0 bg-white z-10 border-r border-slate-100 font-bold text-slate-700 shadow-[2px_0_5px_rgba(0,0,0,0.05)] text-base">
                            <div className="flex flex-col">
                              <span>{doc.name}</span>
                              {savedCycle?.memo && (
                                <span className="text-xs text-amber-600 font-normal leading-tight mt-0.5">
                                  {savedCycle.memo}
                                </span>
                              )}
                            </div>
                          ...
                          <td className="px-2 py-3 text-center bg-teal-50/20 text-teal-700 text-xs border-r border-teal-50 whitespace-nowrap">
                            {savedCycle
                              ? `${savedCycle.startDate.slice(5)}~${savedCycle.endDate.slice(5)}`
                              : "全月"}
                          </td>
                          ...
                          {(() => {
                            ...
                            return stations
                              ...
                              .map((st) => {
                                ...
                                return (
                                  <td
                                    key={`${doc.id}-${displayStationName}-${st.location}`}
                                    className={`px-2 py-3 text-center border-r border-gray-50 text-base ${count > 0 ? "font-bold text-slate-700 bg-slate-50/50" : "text-gray-300"}`}
                                  >
                                    {count > 0 ? count : "-"}
                                  </td>
                                );
            ...
            <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center bg-teal-50/50">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  ...
                  {
                    ["日", "一", "二", "三", "四", "五", "六"][
                      new Date(selectedCell.date).getDay()
                    ]
                  }
                </p>
              </div>
              <button
                ...
                className="text-gray-400 hover:text-gray-600 bg-white p-1 rounded-full shadow-sm hover:shadow transition-all"
              >
                <X size={20} />
              </button>
            </div>
                ...
                <label className="text-xs font-bold text-gray-500 uppercase flex justify-between">
                  ...
                  {editData.station && (
                    <span
                      ...
                      onClick={() => setEditData({ ...editData, station: "" })}
                    >
                      清除分配
                    </span>
                  )}
                ...
                <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto p-1">
                  ...
                  {Array.from(new Set(stations.map((s) => s.name)))
                    ...
                    .map((stationName) => {
                      ...
                      const associatedLocs = stations.filter(
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            const nextLoc = isUniqueLoc
                              ? associatedLocs[0].location
                              : editData.location;
                            setEditData({
                              ...editData,
                              station: stationName,
                  ...
                  {LOCATIONS.map((loc) => (
                    <button
                      ...
                      onClick={() =>
                        setEditData({
                          ...editData,
                          location: editData.location === loc ? "" : loc,
                        })
                      }
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditData({ ...editData, workTime: e.target.value })
                    }
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm border border-gray-100 animate-in zoom-in-95 duration-200 flex flex-col max-h-[80vh]">
            <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center bg-teal-50/50 shrink-0">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {assignModal.date} • {assignModal.station} (
                  {assignModal.location})
                </p>
              </div>
              <button
            ...
            <div className="p-2 overflow-y-auto">
              {(() => {
                ...
                const scheduledSkilled: {
                  doc: Doctor;
                  stationName: string;
                  isUnavailable: boolean;
                  offReason?: string;
                }[] = [];
                const unscheduledSkilled: {
                  doc: Doctor;
                  isUnavailable: boolean;
                  offReason?: string;
                }[] = [];
                ...
                skilledDocs.forEach((d) => {
                  ...
                  const isWorking =
                    assignedSt &&
                    !["X", "OFF", "休假", "Unassigned", "未分配"].includes(
                      assignedSt,
                    );
                  ...
                  if (isWorking) {
                    scheduledSkilled.push({
                      doc: d,
                      stationName: assignedSt,
                      isUnavailable,
                      offReason,
                    });
                  } else {
                    unscheduledSkilled.push({
                      doc: d,
                      isUnavailable,
                      offReason,
                    });
                    ...
                    {unscheduledSkilled.length > 0 && (
                      <div className="mb-4">
                        ...
                        <div className="grid grid-cols-2 gap-2 p-1">
                          {unscheduledSkilled.map(
                            ({ doc, isUnavailable, offReason }) => (
                              <button
                                key={doc.id}
                              ...
                              >
                                <div
                                  className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs shrink-0 ${isUnavailable ? "bg-gray-400" : "bg-emerald-500 group-hover:scale-110"}`}
                                >
                                  {doc.alias}
                                </div>
                                <div className="min-w-0">
                                  <div
                                    className={`font-bold text-sm truncate ${isUnavailable ? "text-gray-400" : "text-gray-700"}`}
                                  >
                                    {doc.name}
                                  </div>
                                  <div
                                    className={`text-[10px] font-medium ${isUnavailable ? "text-gray-400" : "text-emerald-500"}`}
                                  >
                                    {offReason || "空閒中"}
                                  </div>
                                </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 p-1">
                          {scheduledSkilled.map(
                            ({
                              ...
                              >
                                <div
                                  className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs shrink-0 ${isUnavailable ? "bg-gray-400" : "bg-orange-400"}`}
                                >
                                  {doc.alias}
                                </div>
                                <div className="min-w-0">
                                  ...
                                  <div
                                    className={`text-[10px] font-bold whitespace-nowrap overflow-hidden text-ellipsis ${isUnavailable ? "text-gray-400" : "text-orange-500"}`}
                                  >
                                    {offReason
                                      ? offReason
                                      : `已排在: ${stationName}`}
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2 p-1">
                          {otherDocs.map((doc) => {
                            ...
                            const isExcluded = assignedSt === "X";
                            const isPermanentlyOff =
                              doc.excludedDays?.includes(dayOfWeek);
                            const isUnavailable =
                              isExcluded || isPermanentlyOff;
                            const offReason = isExcluded
                              ? "當天禁排"
                              : isPermanentlyOff
                                ? "固定不報到"
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="min-w-0">
                                  <div className="font-bold text-gray-400 text-sm truncate">
                                    {doc.name}
                                  </div>
                                  <div className="text-[10px] text-gray-400 truncate">
                                    {offReason ||
                                      (isWorking ? `已排: ${assignedSt}` : "")}
                                  </div>
                                </div>
                              </button>
                            );
                      ...
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {(() => {
                              const fixedCount = dateRange.reduce(
                                (count, date) => {
                                  const dayOfWeek = new Date(date).getDay();
                                  return (
                                    count +
                                    (doctor.fixedShifts?.filter(
                                      (fs) => fs.dayOfWeek === dayOfWeek,
                                    ).length || 0)
                                  );
                                },
                                0,
        ...
        message={
          <>
            ...
            <p className="text-xs text-gray-500 mt-2">
              範圍：{dateRange[0]} 至 {dateRange[dateRange.length - 1]}
              <br />
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm border border-gray-100 animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <div className="p-4 overflow-y-auto flex-1 space-y-2">
              ...
              {tempSpecialties.map((spec, idx) => (
                <div
                  key={spec}
                  className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg shadow-sm hover:border-teal-300 transition-colors"
                >
                  <div className="flex items-center gap-3">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-slate-200 flex flex-col max-h-[90vh] overflow-hidden">
            ...
            <div className="p-4 bg-slate-50 border-b border-slate-100 gap-3 flex flex-col">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  onClick={() => {
                    const currentShifts = isSimulationMode
                      ? simulatedShifts || []
                      : activeShifts;
                    const hasShiftIds = new Set(
                      currentShifts
                        .filter(
                          (s) =>
                            dateRange.includes(s.date) &&
                    ...
                    const idsToHide = doctors
                      .map((d) => d.id)
                      .filter((id) => !hasShiftIds.has(id));
                    ...
                    return (
                      <label
                        ...
                        className={`flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all ${
                          isVisible
                            ? "bg-teal-50/50 hover:bg-teal-50 border border-teal-100"
                            : "bg-white hover:bg-slate-50 border border-transparent"
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${isVisible ? "bg-teal-100 text-teal-700" : "bg-slate-100 text-slate-400"}`}
                          >
                            {doc.alias || doc.name.charAt(0)}
                          </div>
                          <div>
                            <div
                              className={`font-bold text-sm ${isVisible ? "text-teal-900" : "text-slate-500"}`}
                            >
                              {doc.name}
                            </div>
                        ...
                        <input
                          ...
                          onChange={() => {
                            if (isVisible) {
                              ...
                              setHiddenDoctorIds(
                                hiddenDoctorIds.filter((id) => id !== doc.id),
                              );
                            }
                          }}
                        />
