      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 border border-gray-100 scale-100 animate-in zoom-in-95 duration-200 relative">
        ...
        <div className="flex items-start gap-4 mb-4">
             <div className={`p-3 rounded-full shrink-0 ${
                 confirmColor === 'red' ? 'bg-red-50 text-red-600' : 
        ...
        <div className="flex gap-3 mt-6">
          <button 
            onClick={onClose}
            className="flex-1 px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg font-bold text-sm transition-colors"
          >
            取消
          </button>
          <button 
            onClick={() => { onConfirm(); onClose(); }}
            className={`flex-1 px-4 py-2.5 text-white rounded-lg font-bold text-sm shadow-sm transition-colors ${
                confirmColor === 'red' 
