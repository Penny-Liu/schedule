...
export const countNonSundayDays = (ref: Date, target: Date): number => {
  const totalDays = Math.floor((target.getTime() - ref.getTime()) / 86_400_000);
...
export const getRoleLabel = (role: UserRole): string => {
  switch (role) {
    case "SUPERVISOR" as UserRole:
      return "部門主管";
    case "SYSTEM_ADMIN" as UserRole:
      return "系統管理員";
    case "SCHEDULER" as UserRole:
      return "排班管理員";
    case "PHYSICIAN_ADMIN" as UserRole:
      return "醫師排班管理";
    case "VIEWER" as UserRole:
      return "瀏覽者";
    case "FINANCE" as UserRole:
      return "財會";
    case "HM_SUPERVISOR" as UserRole:
      return "健管主管";
...
export const isUserOnEmploymentPause = (
...
/**
 * Determine whether a shift's specialRoles contains any blocking task
 * (i.e. roles that should prevent leave/approval). We treat
 * "配合銷假" as a non-blocking marker and ignore it here.
 */
