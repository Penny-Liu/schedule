...
class Store {
    ...
    async autoAssignSpecialRoles(startDate: string, endDate: string, targetRoles: string[]) {
        ...
        // This ensures "Re-Schedule" works cleanly without duplicates
        const shiftsToClear = this.getShifts(startDate, endDate).filter(s => s.isAutoGenerated && s.specialRoles.length > 0);
