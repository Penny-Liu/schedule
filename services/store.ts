...
class Store {
  ...
  getUserStatusOnDate(userId: string, dateStr: string): "WORK" | "OFF" {
    ...
    // Only a manual shift is an explicit override. Auto-generated rows are
    // scheduling output (and are reset to "Unassigned" before regeneration),
    // so letting them override the base calendar would freeze an older Group D
    // rotation and make a newly configured cycle appear inconsistent.
    const manualShift = shift && !shift.isAutoGenerated ? shift : undefined;

    // Explicit manual override: If assigned to OFF, it's OFF.
    if (manualShift && manualShift.station === SYSTEM_OFF) {
      return "OFF";
    }

    // Explicit manual override: If record exists and NOT OFF, it is WORK.
