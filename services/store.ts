...
class Store {
  ...
  async autoSchedule(startDate: string, endDate: string) {
    ...
    // Batch clear (conceptually) - actually we just iterate and update
    ...
    // Order: Dazhi -> Floor Control -> Remote -> US -> MR -> Others
    ...
    // Reset start date for main loop
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        ...
        // Identify ALL slots needed for this day
        // This flattens requirements: e.g., if US needs 3, we add 'US', 'US', 'US' to the list
        ...
        // Sort slots based on Strict Priority
        ...
        // Get Available Workers (supply)
        // Filter out those who are manually locked (not auto-generated and assigned to a station)
        const allWorkingUsers = this.users.filter(user => {
            ...
            // If user has a manually assigned station (not auto), they are taken.
            // UNASSIGNED or Auto-Generated means they are available for reallocation.
        ...
        // --- Monte Carlo / Retry Logic ---
        // Try multiple times to fill the schedule randomly and pick the best result (least empty slots)
        // This avoids the issue where a person capable of 'A' is taken by 'B' early on, leaving 'A' empty.
        ...
        for (let attempt = 0; attempt < 50; attempt++) {
             ...
             // Shuffle candidates for fairness and variation
             const pool = [...allWorkingUsers].sort(() => Math.random() - 0.5);
             const slots = [...slotsNeeded]; // Copy needed slots

             // Try to fill slots in order
             const unfilledSlots: string[] = [];

             for (const slot of slots) {
                 // Find a candidate in the pool who can do this slot
                 // AND check conflict constraints (e.g. Opening/Late shouldn't do Floor Control if restricted)
                 const candidateIndex = pool.findIndex(u => {
                     // Basic Capability Check
                     const isCertified = u.capabilities?.includes(slot);
                     const isLearning = u.learningCapabilities?.includes(slot); // Auto schedule usually prefers certified, but allowing learners if needed? 
                     // Let's assume Auto Schedule only assigns CERTIFIED users to critical roles to be safe, 
                     // or follows the logic: must be in capabilities.
                     if (!isCertified && !isLearning) return false;

                     // Special Role Conflicts Check
                 ...
                 if (candidateIndex >= 0) {
                     ...
                     pool.splice(candidateIndex, 1); // Remove from pool
                 } else {
                     unfilledSlots.push(slot);
                 }
             }

             // Handle "Leftovers" (Pool users) -> Assign to remaining pool (Admin/Tech Support) if they have the skill and slots are open?
             // Or just unassigned.
             // But we want to assign remaining users to something if possible, or just track the core requirements.
             
             // Calculate Score: Lower unfilled count is better.
             if (unfilledSlots.length < minUnfilledCount) {
                 minUnfilledCount = unfilledSlots.length;
                 bestAllocation = currentAllocation;
                 
                 // If we found a perfect match (0 unfilled), we can stop early?
                 // Maybe run a few more to ensure randomness, but if 0 is reached, it's good.
        ...
        // Handle Unassigned Users (Leftovers from the best run)
        // Assign them to 'Unassigned' or specific overflow logic if needed
        // The previous logic for overflow was: Tech Support / Admin / Floor Control.
        // We can do a quick pass for remaining unassigned users to fill non-required spots if desired, 
        // but user specifically asked to "Fill Priority Stations".
        // Any user NOT in bestAllocation is effectively "Unassigned" (or kept their manually assigned role if filtered out earlier).
        
        // Find users who were in 'allWorkingUsers' but NOT in 'bestAllocation'
        ...
        for (const user of leftovers) {
            // Check if they can fill any "Overflow" stations that don't have strict requirements or if requirements were met
            // For now, reset them to UNASSIGNED so they don't hold stale data, 
            // OR try to put them in Tech Support if capable?
            // Let's reset to UNASSIGNED to be clean, unless they have a manual override (which wouldn't be in allWorkingUsers loop if locked).
             const existingShiftIdx = this.shifts.findIndex(s => s.userId === user.id && s.date === dateStr);
             if (existingShiftIdx >= 0) {
                 const s = this.shifts[existingShiftIdx];
                 s.station = StationDefault.UNASSIGNED; // Reset
                 s.isAutoGenerated = true;
                 await this.upsertShift(s);
             }
        }
