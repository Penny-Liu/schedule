...
class Store {
  ...
  async loadDataForMonth(year: number, month: number) {
    ...
    if (workloadsRes.data) {
      const parsed = workloadsRes.data.map((w: any) => ({
          ...
          mrPostProcessing: w.mrPostProcessing || w.mr_post_processing || 0,
          us: w.us || 0,
          usA: w.usA || w.us_a || 0,
  ...
  async updateWorkload(w: Partial<RadiographerWorkload>) {
    ...
    try {
      ...
      const payload = {
        ...
        mr_small: w.mrSmall,
        mr_post_processing: w.mrPostProcessing ?? 0,
        us: w.us,
        us_a: w.usA,
      ...
      const fullWorkload: RadiographerWorkload = {
        ...
        mrSmall: w.mrSmall || 0,
        mrPostProcessing: w.mrPostProcessing || 0,
        us: w.us || 0,
        usA: w.usA || 0,
    ...
    try {
      const payloads = records.map(w => {
        ...
        const fieldMapping: Record<string, string> = {
          ...
          mrMedium: "mr_medium",
          mrSmall: "mr_small",
          mrPostProcessing: "mr_post_processing",
          us: "us",
          usA: "us_a",
          usBreast: "us_breast",
          usHeart: "us_heart",
  ...
  private mapToDbFields(obj: any) {
    const mapping: Record<string, string> = {
      ...
      learningStation: "learning_station",
      learningTeacherId: "learning_teacher_id",
      ...
      isRoleAutoGenerated: "is_role_auto_generated",
      doctorId: "doctor_id",
  ...
  private mapFromDbFields(obj: any) {
    const mapping: Record<string, string> = {
      ...
      learning_station: "learningStation",
      learning_teacher_id: "learningTeacherId",
      ...
      mr_medium: "mrMedium",
      mr_small: "mrSmall",
      mr_post_processing: "mrPostProcessing",
      us_a: "usA",
      us_breast: "usBreast",
      us_heart: "usHeart",
      us_thy: "usThy",
  ...
  async updateLeaveStatus(id: string, status: LeaveStatus, approverId: string) {
    ...
    // Apply every schedule update before marking the request approved. This
    // prevents a request from appearing completed when only part of its swap
    // was persisted.
  ...
  private async applyLeaveToShifts(leave: LeaveRequest) {
    ...
    const persistLeaveShift = async (candidate: Shift) => {
      const existing = this.shifts.find(
        (shift) =>
          shift.userId === candidate.userId && shift.date === candidate.date,
      );
      const shiftToPersist =
        candidate.station === StationDefault.UNASSIGNED &&
        existing &&
        existing.station !== SYSTEM_OFF
          ? { ...existing, isAutoGenerated: false }
          : candidate;
      const { error } = await this.upsertShift(shiftToPersist);
      if (error) throw error;
    };

    if (leave.type === LeaveType.SWAP_SHIFT) {
      const swapUpdates = buildSwapLeaveShiftUpdates(leave);
      if (swapUpdates.length !== 4) {
        throw new Error("換假資料不完整，未更新排班");
      }
      for (const shift of swapUpdates) {
        await persistLeaveShift(shift);
      }
      return;
    }
    ...
    for (
      ...
      if (
        ...
        if (requestorShift && leave.targetUserId) {
          ...
          await persistLeaveShift(newTargetShift);
