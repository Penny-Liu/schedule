    ...
    private async applyLeaveToShifts(leave: LeaveRequest) {
        ...
        for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
            ...
            if (leave.type === LeaveType.PRE_SCHEDULED || leave.type === LeaveType.LONG_LEAVE) {
                ...
                // Safer: Delete the shift record so it falls back to Base Status (e.g. Roster Cycle)
                await this.deleteShift(leave.userId, dateStr);
                ...
                if (leave.targetUserId) {
                    // Try to preserve target's existing station if possible (though "Swap" implies filling in)
                    // If target is filling for someone, they are usually "Unassigned" (meaning working freely) or take the spot?
                    // Standard practice: Set to Unassigned (Working)
                    await this.upsertShift({
